...
export function useFormValidation<T extends Record<string, any>>(
  ...
  // 필드 blur 처리
  const blurField = useCallback((fieldName: keyof T) => {
    setTouchedFields(prev => new Set(prev).add(fieldName))
...
export const commonValidationRules = {
   ...
   * 필수 입력 검사
   */
  required: (fieldName: string = '필드') => (value: any) => {
   ...
   * 문자열 길이 검사
   */
  stringLength: (min?: number, max?: number, fieldName: string = '값') => (value: string) => {
   ...
   * 숫자 범위 검사
   */
  numberRange: (min?: number, max?: number, fieldName: string = '값') => (value: number) => {
   ...
   * 여러 validation 조합
   */
  combine: (...validations: Array<(value: any) => string | null>) => (value: any) => {

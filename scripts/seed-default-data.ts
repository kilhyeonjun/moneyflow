...
import { defaultAssetCategories, defaultTransactionCategories } from '../lib/seed-data'
...
async function seedDefaultTransactionCategories() {
  ...
  for (const categoryData of defaultTransactionCategories) {
    ...
    if (!existing) {
      ...
      console.log(`✅ 거래 카테고리 생성: ${categoryData.name} (${categoryData.transactionType})`)
    } else {
      console.log(`⚠️  거래 카테고리 이미 존재: ${categoryData.name} (${categoryData.transactionType})`)
    }
  }
}

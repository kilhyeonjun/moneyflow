...
import { Select, SelectItem, type SelectProps, type Selection } from '@heroui/react'
...
export interface ValidatedSelectProps extends Omit<SelectProps, 'errorMessage' | 'isInvalid' | 'children'> {
  /**
   * 선택 옵션 리스트
   */
   ...
   */
  onSelectionChangeWithValidation?: (keys: Set<string>, error: string | null) => void
...
export default function ValidatedSelect({
  ...
  // validation 실행
  const validateValue = useCallback((keys: Set<string>): string | null => {
  ...
  // renderValue 처리
  const defaultRenderValue = useCallback((items: any[]) => {
  ...
  return (
    <Select
      ...
      // 필수 필드 스타일링
      classNames={{
        label: isRequired ? 'text-foreground after:content-["*"] after:text-danger after:ml-0.5' : undefined,
        ...props.classNames
      }}
    >
...
export const selectValidationRules = {
   ...
   * 필수 선택 검사
   */
  required: (fieldName: string = '항목') => (value: string) => {
   ...
   * 특정 값들 중 하나인지 검사
   */
  oneOf: (allowedValues: string[], fieldName: string = '값') => (value: string) => {
    return !allowedValues.includes(value) ? `올바른 ${fieldName}을(를) 선택해주세요` : null
  },
   ...
   * 여러 validation 조합
   */
  combine: (...validations: Array<(value: string) => string | null>) => (value: string) => {

...
export interface ValidatedInputProps extends Omit<InputProps, 'errorMessage' | 'isInvalid'> {
  /**
   * 유효성 검사 함수
   * @param value - 입력된 값
...
export default function ValidatedInput({
  ...
  const handleBlur = useCallback((e: React.FocusEvent<HTMLInputElement>) => {
    ...
    // 원래 onBlur 호출
    props.onBlur?.(e)
  }, [realTimeValidation, validateValue, onValueChangeWithValidation, props.onBlur])
...
export const validationRules = {
   ...
   * 필수 입력 검사
   */
  required: (fieldName: string = '필드') => (value: string) => {
   ...
   * 최소 길이 검사
   */
  minLength: (min: number, fieldName: string = '값') => (value: string) => {
    return value.length < min ? `${fieldName}은(는) 최소 ${min}자 이상 입력해주세요` : null
  },

  /**
   * 최대 길이 검사
   */
  maxLength: (max: number, fieldName: string = '값') => (value: string) => {
    return value.length > max ? `${fieldName}은(는) 최대 ${max}자까지 입력 가능합니다` : null
  },

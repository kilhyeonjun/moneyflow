...
export default function PaymentMethodList({
  ...
  const loadPaymentMethods = async () => {
    try {
      ...
      if (errorMessage) {
        showToast.error(`결제수단 목록을 불러오는데 실패했습니다: ${errorMessage}`)
      }
    } finally {
      setLoading(false)
    }
  ...
  const handleUpdatePaymentMethod = async (data: PaymentMethodFormData) => {
    ...
    setPaymentMethods(prev =>
      prev.map(pm => pm.id === data.id ? updatedPaymentMethod as PaymentMethodData : pm)
    )
  }
  ...
  const handleToggleStatus = async (paymentMethodId: string) => {
    try {
      const result = await togglePaymentMethodStatus(paymentMethodId, organizationId)
      const updatedPaymentMethod = handleServerActionResult(result)
      
      setPaymentMethods(prev =>
        prev.map(pm => pm.id === paymentMethodId ? updatedPaymentMethod as PaymentMethodData : pm)
      )
      
      showToast.success(
        updatedPaymentMethod.isActive ? '결제수단이 활성화되었습니다' : '결제수단이 비활성화되었습니다'
      )
    } catch (error) {
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-center">
                  ...
                  <div>
                    <div className="text-2xl font-bold text-orange-600">
                      {paymentMethods.reduce((sum, pm) => sum + pm.transactionCount, 0)}
                    </div>
